...
pg_jwt-13.so:
	...
	cc -shared -o pg_jwt-13.so jwt_func.o jwt_def.o -lcrypto -lssl -lstdc++
...
pg_jwt-14.so:
	...
	cc -shared -o pg_jwt-14.so jwt_func.o jwt_def.o -lcrypto -lssl -lstdc++
...
pg_jwt-15.so:
	...
	cc -shared -o pg_jwt-15.so jwt_func.o jwt_def.o -lcrypto -lssl -lstdc++
...
pg_jwt-16.so:
	...
	cc -shared -o pg_jwt-16.so jwt_func.o jwt_def.o -lcrypto -lssl -lstdc++
...
all: pg_jwt-13.so pg_jwt-14.so pg_jwt-15.so pg_jwt-16.so

install: all
	@test -d $(INSTALL_ROOT)/opt/pg_jwt/ || mkdir -p $(INSTALL_ROOT)/opt/pg_jwt/
	$(INSTALL_PROG) ./pg_jwt.control     $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt.control
	$(INSTALL_PROG) ./pg_jwt--1.0.0.sql  $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt--1.0.0.sql
	$(INSTALL_PROG) ./pg_jwt-13.so       $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt-13.so
	$(INSTALL_PROG) ./pg_jwt-14.so       $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt-14.so
	$(INSTALL_PROG) ./pg_jwt-15.so       $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt-15.so
	$(INSTALL_PROG) ./pg_jwt-16.so       $(INSTALL_ROOT)/opt/pg_jwt/pg_jwt-16.so


clean:
	rm ./*.o ./pg_jwt-13.so ./pg_jwt-14.so ./pg_jwt-15.so ./pg_jwt-16.so
