...
// C++11 requires the body of a constexpr constructor to be empty
	...
	namespace algorithm {
		...
		struct ecdsa {
             ...
             */
            void verify2(const std::string& data, const std::string& signature, std::error_code& ec) const {
                ec.clear();
                //std::string der_signature = signature;
                if (ec) { return; }
                ...
                unsigned char* der_sig_data = reinterpret_cast<unsigned char*>(const_cast<char*>(signature.data()));
#else
                const unsigned char* der_sig_data = reinterpret_cast<const unsigned char*>(signature.data());
#endif
                auto res =
                    EVP_DigestVerifyFinal(ctx.get(), der_sig_data, static_cast<unsigned int>(signature.length()));
                if (res == 0) {
                    ec = error::signature_verification_error::invalid_signature;
                    return;
                }
